...
client.on('interactionCreate', async (interaction) => {
  ...
  try {
    ...
    if (commandName === 'ask' || commandName === 'chat') {
      ...
      console.log("[v0] /ask command received with input:", input);
      ...
      console.log("[v0] AI response received:", response.text.substring(0, 100));
    ...
    console.error("[v0] Error handling slash command:", error.message);
    console.error("[v0] Full error:", error);
    if (interaction.deferred) {
      await interaction.editReply('Sorry, I encountered an error: ' + error.message);
    } else {
      await interaction.reply('Sorry, I encountered an error: ' + error.message);
    }
  }
});
